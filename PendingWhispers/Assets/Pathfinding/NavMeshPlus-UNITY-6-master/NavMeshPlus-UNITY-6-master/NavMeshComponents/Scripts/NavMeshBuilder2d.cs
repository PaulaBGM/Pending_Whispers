    ...
    NavMeshBuilder2dState builder)
{
    ...
    var size = new Vector3(
        tilemap.layoutGrid.cellSize.x,
        tilemap.layoutGrid.cellSize.y,
        0
    );
    ...
    // Obtener Mesh del prefab de forma segura
    if (builder.useMeshPrefab != null)
    {
        var meshFilter = builder.useMeshPrefab.GetComponent<MeshFilter>();

        if (meshFilter == null)
        {
            Debug.LogError(
                $"NavMeshPlus: El Use Mesh Prefab '{builder.useMeshPrefab.name}' " +
                "no tiene componente MeshFilter. Se usará Box en su lugar."
            );
        }
        else
        {
            ...
            if (sharedMesh == null)
            {
                Debug.LogError(
                    $"NavMeshPlus: El Use Mesh Prefab '{builder.useMeshPrefab.name}' " +
                    "tiene MeshFilter pero no tiene ningún Mesh asignado. " +
                    "Se usará Box en su lugar."
                );
            }
            else
            {
    ...
    for (int i = bound.xMin; i < bound.xMax; i++)
    {
        for (int j = bound.yMin; j < bound.yMax; j++)
        {
            ...
            // Si CollectTile intentó crear un Mesh pero no consiguió uno,
            // NO añadimos el source a la lista.
            if (src.shape == NavMeshBuildSourceShape.Mesh &&
                src.sourceObject == null)
            {
                Debug.LogWarning(
                    $"NavMeshPlus: Se ha omitido un Mesh inválido en Tilemap " +
                    $"'{tilemap.name}', celda {vec3int}."
                );
            ...
            {
                src.area = tileModifier.overrideArea
                    ? tileModifier.area
                    : area;
            }
            ...
            var instantiatedObject = tilemap.GetInstantiatedObject(vec3int);

            if (instantiatedObject != null)
            {
                builder.lookupCallback?.Invoke(instantiatedObject, src);
            }
        }
    }
}
        ...
        private static void CollectTile(Tilemap tilemap, NavMeshBuilder2dState builder, Vector3Int vec3int, Vector3 size, Mesh sharedMesh, Quaternion rot, ref NavMeshBuildSource src)
        {
            ...
            else if (builder.useMeshPrefab != null && sharedMesh != null)
            {
                src.transform = Matrix4x4.TRS(
                    Vector3.Scale(
                        tilemap.GetCellCenterWorld(vec3int),
            ...
            {
                // Si no hay Mesh válido, usamos Box.
                src.transform = GetCellTransformMatrix(tilemap, builder.overrideVector, vec3int);
