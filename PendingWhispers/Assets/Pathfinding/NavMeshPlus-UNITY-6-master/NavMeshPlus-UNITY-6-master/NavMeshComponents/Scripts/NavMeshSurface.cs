...
// Elimina NavMeshBuildSources de tipo Mesh que no tienen un Mesh válido.
// Unity lanza:
// "RuntimeNavMeshBuilder. Source Mesh missing at index: X"
// si alguno llega a UpdateNavMeshDataAsync() con sourceObject == null.
            int removedInvalidMeshes = 0;
            ...
            for (int i = sources.Count - 1; i >= 0; i--)
            {
                ...
                if (source.shape == NavMeshBuildSourceShape.Mesh &&
                    source.sourceObject as Mesh == null)
                {
                    Debug.LogWarning(
                        $"NavMeshPlus: eliminando Mesh inválido en source index {i}. " +
                        $"El source tenía shape Mesh pero sourceObject era null."
                    );

                    sources.RemoveAt(i);
                    removedInvalidMeshes++;
                }
            }

            if (removedInvalidMeshes > 0)
            {
                Debug.LogWarning(
                    $"NavMeshPlus: se eliminaron {removedInvalidMeshes} NavMeshBuildSource(s) con Mesh inválido."
                );
            }
            ...
            foreach (var src in sources)
            {
                switch (src.shape)
                {
                    case NavMeshBuildSourceShape.Mesh:
                    {
                        ...
                        {
                            Debug.LogWarning(
                                $"NavMeshPlus: Se encontró una fuente Mesh sin Mesh válido. " +
